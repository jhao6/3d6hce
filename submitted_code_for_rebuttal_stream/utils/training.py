...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    if args.csv_log:
        ...
        task_perf_path = os.path.join(args.output_dir, "results", dataset.SETTING, dataset.NAME, model.NAME, args.experiment_id, f'task_performance.txt')
        ...
        for epoch in range(args.n_epochs):
            for i, data in enumerate(train_loader):
                ...
                if hasattr(dataset.train_loader.dataset, 'logits'):
                    ...
                    inputs, labels = inputs.to(model.device), labels.to(torch.int64).to(
                        model.device)
                    not_aug_inputs = not_aug_inputs.to(model.device)
